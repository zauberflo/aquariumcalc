...
# --- VARIABLEN VOR SIDEBAR DEFINIEREN ---
...
if "kh_dosis_live" not in st.session_state:
    st.session_state.kh_dosis_live = float(setup_values["KH_Dosis"])
if "ca_dosis_live" not in st.session_state:
    st.session_state.ca_dosis_live = float(setup_values["CA_Dosis"])
...
with st.sidebar:
    ...
    s_kh_d = st.number_input(f"Dosis {s_brand_kh}", value=st.session_state.kh_dosis_live, format="%.1f", key="sb_kh_input")
    s_ca_d = st.number_input(f"Dosis {s_brand_ca}", value=st.session_state.ca_dosis_live, format="%.1f", key="sb_ca_input")
    
    # Synchronisiere Änderungen aus der Sidebar in den Session State
